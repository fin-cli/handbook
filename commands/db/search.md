...
Defaults to searching through all tables registered to $findb. On multisite, this default is limited to the tables for the current site.
...
[\--network]
: Search through all the tables registered to $findb in a multisite install.

[\--all-tables-with-prefix]
: Search through all tables that match the registered table prefix, even if not registered on $findb. On one hand, sometimes plugins use tables without registering them to $findb. On another hand, this could return tables you don't expect. Overrides --network.

[\--all-tables]
: Search through ALL tables in the database, regardless of the prefix, and even if not registered on $findb. Overrides --network and --all-tables-with-prefix.
...
### EXAMPLES
    ...
    $ fin db search wordpress-develop
    ...
    $ fin db search example.com --url=example.com/foo
    fin_2_comments:comment_author_url
    1:https://example.com/
    fin_2_options:option_value
        ...

    # Search through the database for the 'https?://' regular expression, printing stats.
    $ fin db search 'https?://' --regex --stats
    fin_comments:comment_author_url
    1:https://wordpress.org/
        ...
    Success: Found 99146 matches in 10.752s (10.559s searching). Searched 12 tables, 53 columns, 1358907 rows. 1 table skipped: fin_term_relationships.

    # SQL search database table 'fin_options' where 'option_name' match 'foo'
    fin db query 'SELECT * FROM fin_options WHERE option_name like "%foo%"' --skip-column-names
    +----+--------------+--------------------------------+-----+
    | 98 | foo_options  | a:1:{s:12:"_multiwidget";i:1;} | yes |
    | 99 | foo_settings | a:0:{}                         | yes |
    +----+--------------+--------------------------------+-----+

    # SQL search and delete records from database table 'fin_options' where 'option_name' match 'foo'
    fin db query "DELETE from fin_options where option_id in ($(fin db query "SELECT GROUP_CONCAT(option_id SEPARATOR ',') from fin_options where option_name like '%foo%';" --silent --skip-column-names))"

    # Search for a string and print the result as a table
    $ fin db search https://localhost:8889 --format=table --fields=table,column,match
    +------------+--------------+-----------------------------+
    | table      | column       | match                       |
    +------------+--------------+-----------------------------+
    | fin_options | option_value | https://localhost:8889      |
    | fin_options | option_value | https://localhost:8889      |
    | fin_posts   | guid         | https://localhost:8889/?p=1 |
    | fin_users   | user_url     | https://localhost:8889      |
    +------------+--------------+-----------------------------+

    # Search for a string and get only the IDs (only works for a single table)
    $ fin db search https://localhost:8889 fin_options --format=ids
    1 2
