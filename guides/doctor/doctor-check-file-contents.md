...
As an example, here are two checks using `File_Contents`, one which ensures sessions aren't used by plugins and the other which ensures `$_SERVER['SERVER_NAME']` isn't used in `fin-config.php`:

    file-sessions:
      check: File_Contents
      options:
        regex: .*(session_start|\$_SESSION).*
        only_fin_content: true
    file-server-name-fin-config:
      check: File_Contents
      options:
        regex: define\(.+FIN_(HOME|SITEURL).+\$_SERVER.+SERVER_NAME
        path: fin-config.php


Run together, you might see:

    $ fin doctor check --config=file-contents.yml --all
    +----------------------------+---------+-----------------------------------------------------------------------------------------+
    | name                       | status  | message                                                                                 |
    +----------------------------+---------+-----------------------------------------------------------------------------------------+
    | file-sessions              | success | All 'php' files passed check for '.*(session_start|\$_SESSION).*'.                      |
    | file-server-name-fin-config | success | All 'php' files passed check for 'define\(.+FIN_(HOME|SITEURL).+\$_SERVER.+SERVER_NAME'. |
    +----------------------------+---------+-----------------------------------------------------------------------------------------+
...
* 'path'. Check a specific file path. Value should be relative to `ABSPATH` (e.g. 'fin-content' or 'fin-config.php').
* 'only\_fin\_content': Only check the fin-content directory.
